...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("--config",required=True); parser.add_argument("--checkpoint",required=True); parser.add_argument("--split",choices=["image","track"],required=True); parser.add_argument("--partition",choices=["val","test"],default="test"); parser.add_argument("--dry-run",action="store_true"); parser.add_argument("--overwrite",action="store_true"); args=parser.parse_args()
    cfg=yaml.safe_load(Path(args.config).read_text())
    if args.dry_run: print({"checkpoint":args.checkpoint,"split":args.split,"partition":args.partition,"outer_folds_locked":True}); return
    ...
    transform=transforms.Compose([transforms.Resize(256),transforms.CenterCrop(int(cfg["image_size"])),transforms.ToTensor(),transforms.Normalize([.485,.456,.406],[.229,.224,.225])]); dataset=F4KDataset(records,transform,class_ids=class_ids)
    ...
    output=Path(args.checkpoint).parent/"evaluation"; atomic_csv_dump(frame,output/f"per_image_{args.partition}.csv",args.overwrite); atomic_csv_dump(per_track,output/f"per_track_{args.partition}.csv",args.overwrite); atomic_json_dump(metrics,output/f"metrics_{args.partition}.json",args.overwrite); print(json.dumps(metrics,indent=2))
