...
def restore_rng(state: dict) -> None:
    random.setstate(state["python"]); np.random.set_state(state["numpy"]); torch.set_rng_state(state["torch"].detach().cpu().to(dtype=torch.uint8).contiguous())
    if "cuda" in state and torch.cuda.is_available(): torch.cuda.set_rng_state_all([item.detach().cpu().to(dtype=torch.uint8).contiguous() for item in state["cuda"]])
...
def main() -> None:
    ...
    frozen = json.loads((ROOT / cfg["class_ids_path"]).read_text(encoding="utf-8"))
    class_ids = [str(value) for value in frozen["class_ids"]]
    ...
    train_set = MaskContrastiveDataset(train_records, MaskContrastiveTransform(size, tuple(cfg["pretrain_augmentation"]["crop_scale"])), class_ids, blur_kernel=int(cfg["views"]["blur_kernel"]), blur_sigma=float(cfg["views"]["blur_sigma"]), feather_radius=int(cfg["views"]["feather_radius"]))
    ...
    if output.exists() and any(output.iterdir()) and not args.resume: raise FileExistsError(f"Refusing to overwrite {output}")
    output.mkdir(parents=True, exist_ok=True); history = []; resume_state = None
    if args.resume:
        resume_state = torch.load(args.resume, map_location=device, weights_only=False)
        expected = {"fold": args.fold, "seed": args.seed, "stage": resume_state.get("stage"), "config_sha256": sha256(cfg_path), "outer_train_sha256": sha256(train_path), "inner_dev_sha256": sha256(dev_path), "class_ids": class_ids, "outer_test_accessed": False, "official_test_accessed": False, "initialization_sha256": initialization_sha, "view_schema_sha256": sha256(ROOT / "datasets" / "mask_contrastive_dataset.py")}
        for key, value in expected.items():
            if key == "stage": continue
            if resume_state.get(key) != value: raise ValueError(f"resume provenance mismatch for {key}")
        model.load_state_dict(resume_state["model"], strict=True); history = resume_state["history"]; restore_rng(resume_state["rng_state"])
    resume_stage = resume_state.get("stage") if resume_state else None
    pre_start = int(resume_state["epoch"]) + 1 if resume_stage == "pretrain" else 1
    if resume_stage == "classifier": pre_start = int(cfg["training"]["pretrain_epochs"]) + 1
    if resume_stage == "pretrain":
        pre_opt.load_state_dict(resume_state["optimizer"]); pre_sched.load_state_dict(resume_state["scheduler"]); scaler.load_state_dict(resume_state["scaler"])
    for epoch in range(pre_start, int(cfg["training"]["pretrain_epochs"]) + 1):
    ...
    if resume_stage == "classifier":
        cls_opt.load_state_dict(resume_state["optimizer"]); cls_sched.load_state_dict(resume_state["scheduler"]); scaler.load_state_dict(resume_state["scaler"]); best = float(resume_state["best_inner_dev_accuracy"]); patience = int(resume_state.get("patience", 0)); cls_start = int(resume_state["epoch"]) + 1
    for epoch in range(cls_start, int(cfg["training"]["classifier_epochs"]) + 1):
        ...
        atomic_save(state | {"patience": patience, "sampler_epoch": epoch + int(cfg["training"]["pretrain_epochs"]), "rng_state": capture_rng()}, output / "last.pt"); pd.DataFrame(history).to_csv(output / "training_curve.csv", index=False)
        if patience >= 7: break
